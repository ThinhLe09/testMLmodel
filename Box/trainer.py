...
def train(config):
    ...
    # --- SỬA Ở ĐÂY: Chỉ load train_csv, bỏ test_csv ---
    ...
    # --- SỬA Ở ĐÂY: Chuẩn hóa tên cột question_id cho full_df và df_box_train ---
    for df in [full_df, df_box_train]:
    ...
    # Gộp dữ liệu Box vào full_df trước khi chia
    full_df = pd.merge(full_df, df_box_train[['question_id', 'owl_box']], on='question_id', how='left')

    # --- SỬA Ở ĐÂY: Tạo label encoder từ toàn bộ dữ liệu trước khi chia để tránh mất label ---
    all_answers = full_df['answer'].apply(lambda x: str(x).lower().strip()).unique().tolist()
    ...
    # --- SỬA Ở ĐÂY: Chia tập train ra thành 90% train và 10% validation ---
    train_df, val_df = train_test_split(full_df, test_size=0.1, random_state=42)
    print(f"✅ Đã chia dữ liệu: {len(train_df)} mẫu train, {len(val_df)} mẫu validation.")
    ...
    # --- SỬA Ở ĐÂY: Đổi config['test_img_dir'] thành config['train_img_dir'] vì data val lấy từ train ---
