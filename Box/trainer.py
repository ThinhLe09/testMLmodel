...
def train(config):
    ...
    for df in [train_df, df_box_train]:
    ...
    train_df = pd.merge(
        train_df,
        df_box_train[['question_id', 'owl_box']],
        on='question_id',
        how='left'
    )
    ...
    for epoch in range(config['epochs']):
        ...
        # ===== NO VALIDATION -> dùng train_acc =====
        train_acc = train_correct / train_total
        val_acc = train_acc

        print(f"Epoch {epoch + 1} | Train Acc: {train_acc:.4f}")

        print("    --- Per Q-Type Accuracy ---")
        for t in range(num_q_types):
            tr_acc = (
                train_type_correct[t] / train_type_total[t]
                if train_type_total[t] > 0 else 0
            )
            w = config['type_loss_weights'].get(t, 1.0)
            marker = " <<<" if w > 1.0 else ""
            print(f"    Type {t} (w={w}): Train={tr_acc:.4f}{marker}")
