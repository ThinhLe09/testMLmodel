...
def run_inference_and_save(config, output_file="ket_qua_test_vivqa.csv", batch_size=16):
    ...
    model = PhoBERT_BLIP2_VQA_Hierarchical(num_classes, num_q_types, type_mask_tensor, config)

    # 4. Load Weights
    if os.path.exists(config['save_path']):
        print(f"Loading weights: {config['save_path']}")
    ...
    test_dataset = ViVQAInferenceDataset(test_df, config['test_img_dir'], blip_processor, tokenizer, config)
